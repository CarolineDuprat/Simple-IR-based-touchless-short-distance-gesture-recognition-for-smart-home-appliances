 ...
 * @param all_snapshots all snapshots allowed
...
uint16_t snapshotEqualAll_snapshots (const snapshot_t snapshot,uint16_t counter,snapshots all_snapshots){
    ...
    if (equal(snapshot,all_snapshots.snapshot[counter])==1){
        retour=1;
    }
    return retour;
}
...
uint16_t snapshotResearchAll_snapshots (const snapshot_t snapshot,snapshots all_snapshots){
    ...
    do{
        ...
        retour=snapshotEqualAll_snapshots (snapshot,counter,all_snapshots);
        //if retour==0, Snapshots are differents
        if (retour==0){
            counter++;
        }
    }while((counter<=all_snapshots.nbrsnapshots)&&(retour==0));
 ...
 * @param all_snapshots all snapshots allowed
 * @param transitions transitions_rows and transitions_pool
...
uint16_t snapshotResearchTransitions_pool (const snapshot_t snapshot,uint16_t position,snapshots all_snapshots,transition transitions){
    ...
    counter=transitions.transitions_rows[position];

    //We will compare the next snapshot with which snapshot allows.
    do{
        retour=snapshotEqualAll_snapshots (snapshot,transitions.transitions_pool[counter].snapshotID,all_snapshots);
        if (retour==0){
            counter++;
        }
    }while((counter<=transitions.transitions_rows[position+1])&&(retour==0)); // for all positions allow
...
void snapshotsGestureName (char *lien,snapshots all_snapshots,transition transitions,infoSystem system){
    ...
    while (checkEndFile (informationFile)==0){
       ...
       position= snapshotResearchAll_snapshots (snapshot,all_snapshots);
       printf("position = %d\n",position);
       ...
       if ((position==0)&&(mouvement==1)){
           ...
           gesture_index= gestureSearch(gestureID,system);
           printf("-------------------------------------------------------------------\n");
           if (gesture_index > -1){
               printf("Valid gesture detected\n");
               printf("gesture  : %s\n",system.gesture[gesture_index].name);
           }else{
       ...
       if ((position!=positionAllSnap)&&(position!=0)){
           ...
           positionTransitionPool=snapshotResearchTransitions_pool (snapshot,positionAllSnap,all_snapshots,transitions);
           ...
           if (positionTransitionPool==1000){
               ...
               printf("gesture t : %d\n",transitions.transitions_pool[positionTransitionPool].gestureID);

               // Logical AND between the before gestureID and the gesture allows with this new transition
               gestureID= gestureID & transitions.transitions_pool[positionTransitionPool].gestureID;
               printf("gesture  = %d\n",gestureID);

               // the position of the next snapshot is transitions_pool[positionTransitionPool].snapshotID
               positionAllSnap=transitions.transitions_pool[positionTransitionPool].snapshotID;
...
int16_t gestureSearch(uint16_t idGesture,infoSystem system){
    ...
    for (uint16_t i=0;i<system.numberGesture;i++){
        ...
        if (idGesture==system.gesture[i].id){
            position=i;
        }
    }
    printf("position gesture : %d\n",position);
    return position;
}
